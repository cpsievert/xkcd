---
title: "Finding structure in xkcd comics with Latent Dirichlet Allocation"
...
Unlike many other text classification models, LDA is a special case of a [statistical mixture model](http://en.wikipedia.org/wiki/Mixture_model). In LDA, each document (in our case, comic) is assumed to be a _mixture_ of latent topics. For demonstration purposes, consider the following xkcd comic given a hypothetical model with four topics: "romance" (in red), "sarcasm" (in blue), "math" (in black), and "language" (in green). 
